...
def get_sql_query(requestUrlString, queryString, dbUser):
    ...
    if "/getProducts" in requestUrlString:
        ...
        sqlQuery = "insert into " + tableName + \
            " values ('" + product_name + "' , " + str(product_count) + ")"
    elif "/updateProduct" in requestUrlString:
        product_name = queryString['name'][0]
        product_count = int(queryString['count'][0])
        sqlQuery = "update " + tableName + " set count=" + \
            str(product_count) + " where name = '"+product_name+"'"
...
def read_secret_value(secret_id):
    ...
    secret_client = oci.secrets.SecretsClient(
        config={}, signer=resource_principal_signer)
...
def handler(ctx, data: io.BytesIO = None):
    ...
    # dbuser = read_secret_value(dbuser_ocid)  # "test_user"
    # dbpwd = read_secret_value(dbpwd_ocid)  # "Qwerty12345!"
    dbuser = "test_user"
    dbpwd = "Qwerty12345!"  # read_secret_value 안 됨
